...
[Libre WebUI](https://github.com/libre-webui/libre-webui) is an open-source, local-first AI workspace licensed under **Apache 2.0**, with zero telemetry and no CLA. Sensitive data (chat history, documents, notes, credentials) is encrypted at rest with AES-256-GCM. This tutorial sets up the core workspace: chat with local models through Ollama, document search (RAG) over your own files, interactive artifacts, custom personas, notes, sandboxed coding workspaces (Work), and multi-user access with an admin account. Optional features such as image generation, text-to-speech, and single sign-on can be added later.

![The Libre WebUI chat interface](images/01_libre-webui-interface.en.jpg)

In this tutorial, you will deploy Libre WebUI alongside [Ollama](https://ollama.com) on a GPU dedicated server. By the end, you will have a GPU-accelerated AI workspace accessible from your browser, with all data staying on EU infrastructure.

**What you'll set up:**

* Ollama running LLMs with CUDA GPU acceleration
* Libre WebUI providing a full AI workspace in your browser: chat, document search, artifacts, and sandboxed Work tasks
...
> Replace `<10.0.0.1>` with your actual server IP.
...
* **`CORS_ORIGIN` and `BASE_URL`** - Both must match the URL you use to access the UI in your browser. If they do not match, the page still loads but real-time features such as chat streaming are blocked. Update both if you add a domain name later.
* **`JWT_SECRET` and `ENCRYPTION_KEY`** - Used for session tokens and AES-256-GCM data encryption. You will generate both in a moment. Setting them explicitly is the documented production practice; without a fixed `JWT_SECRET`, a new secret is generated on every start and all users are logged out whenever the container restarts.
* **`/var/run/docker.sock` and `group_add`** - Enable Work, Libre WebUI's sandboxed coding workspaces (Step 6). The Docker socket lets the app create isolated task containers. This grants the application - and therefore every Libre WebUI administrator - control over the Docker host, which is why this tutorial keeps a single administrator account and no self-service registration. Remove the socket line if you do not want Work; everything else keeps working.
...
* **Ollama has no `ports` entry** - It stays private to the Compose network, so the unauthenticated inference API is not reachable from the internet. Libre WebUI talks to it internally, and you pull models with `docker exec`.
* **`volumes`** - Named Docker volumes persist data across container restarts and upgrades. `libre_webui_data` stores the encrypted SQLite database, `libre_webui_temp` is reserved for temporary files, and `ollama_data` stores downloaded model weights.

Save the file (`Ctrl+O`, then `Enter`, then `Ctrl+X` in nano).

Next, generate the secrets in a `.env` file, which Docker Compose reads automatically from the same directory:

```bash
{
  echo "JWT_SECRET=$(openssl rand -hex 32)"
  echo "ENCRYPTION_KEY=$(openssl rand -hex 32)"
  echo "DOCKER_GID=$(docker run --rm -v /var/run/docker.sock:/var/run/docker.sock alpine stat -c '%g' /var/run/docker.sock)"
} > .env
```

* `JWT_SECRET` keeps users logged in across container restarts and upgrades.
* `ENCRYPTION_KEY` must be exactly 64 hexadecimal characters (32 bytes) - `openssl rand -hex 32` produces that. Back up the `.env` file together with the data volume: without the key, the encrypted data cannot be read.
* `DOCKER_GID` is the group that owns the Docker socket, read from inside a container so the value is correct for the container user. Work needs it to access the socket.

Now start both services in detached mode:
...
Pull [Gemma 4 12B](https://ollama.com/library/gemma4), Google's latest open model with vision and tool-calling support, making it an excellent starting point for the GEX44's 20 GB of VRAM:
...
On first launch, Libre WebUI greets you with a short setup wizard that asks you to create the administrator account. Do this right away: until the first account exists, anyone who can reach port 8080 can claim it. Set a strong password and keep the credentials safe. Your instance stays private by default: anyone who signs up after that is placed in a pending state and cannot log in until you approve them in the admin user management panel. To turn public registration off entirely once your admin account exists, add `- ENABLE_SIGNUP=false` to the `environment` section and run `docker compose up -d` again. (Do not set it before the first launch - it also blocks the setup wizard.)

After signing in, select `gemma4:12b` from the model selector in the message box and send a message. Responses should be noticeably fast thanks to GPU acceleration.

You can pull additional models at any time. Here are some recommended models for each GPU tier:

**GEX44 (20 GB VRAM) - these all fit in GPU memory:**

```bash
docker exec ollama ollama pull gemma4:12b         # 7.6 GB - Google's latest open model (vision + tools)
docker exec ollama ollama pull gemma4:e4b         # 9.6 GB - Gemma 4 with audio understanding
docker exec ollama ollama pull mistral-small:24b  # 14 GB  - Mistral's mid-size all-rounder (tools)
docker exec ollama ollama pull qwen3-coder:30b    # 19 GB  - agentic coding specialist (MoE)
```

**GEX131 (96 GB VRAM) - large models that require significantly more memory:**

```bash
docker exec ollama ollama pull llama4:scout       # 67 GB - Meta's multimodal MoE flagship
docker exec ollama ollama pull gpt-oss:120b       # 65 GB - OpenAI's open-weight reasoning model
docker exec ollama ollama pull deepseek-r1:70b    # 43 GB - advanced chain-of-thought reasoning
```
...
**Document search (RAG):** to get semantic search over documents you upload, also pull the embedding model and then enable embeddings in the Libre WebUI settings:

```bash
docker exec ollama ollama pull nomic-embed-text
```

Without an embedding model, document search silently falls back to plain keyword matching.
...
**Running models larger than your VRAM:** You are not limited to models that fit entirely in GPU memory. When a model exceeds the available VRAM, Ollama automatically splits the model layers between the GPU and system RAM. The layers that fit in VRAM run on the GPU at full speed, while the remaining layers are offloaded to RAM and processed on the CPU. This means you can run a 14 GB model like `mistral-small:24b` on the GEX44's 20 GB VRAM entirely on the GPU, but you could also run a 43 GB model like `deepseek-r1:70b` by offloading the extra layers to the GEX44's 64 GB of system RAM. Performance will be slower compared to fully GPU-accelerated inference, but still significantly faster than CPU-only. The GEX131 with its 256 GB of DDR5 RAM is particularly well-suited for running oversized models with partial GPU offloading.

**Pulling models from Hugging Face:** Libre WebUI also includes a built-in integration with [Hugging Face Hub](https://huggingface.co), giving you access to over 1 million models. You can pull GGUF-format models (the quantized format used by Ollama and llama.cpp) directly from Hugging Face through the models page in Libre WebUI, without using the command line. This is useful for finding specialized or community fine-tuned models that are not listed in the Ollama library.

## Step 6 - Run Sandboxed Coding Tasks with Work

Work is Libre WebUI's built-in coding-agent surface. Each Work task gives the selected model its own persistent `/workspace` inside a dedicated, hardened Docker container: it runs as a non-root user with a read-only root filesystem, all Linux capabilities dropped, and CPU, memory, and process limits applied. The model can create and edit files, run commands, and start a browser preview of a generated web app - all inside that sandbox, never on your host directly.

Because the Compose file from Step 4 already mounts the Docker socket and sets `DOCKER_GID`, Work is ready to use. Two things to know:

* **Work is admin-only.** Only administrator accounts see and can use it - another reason to keep registration disabled on a public server.
* **Work needs a model that supports tool calling.** `gemma4` does, so the `gemma4:12b` you pulled in Step 5 works out of the box. For heavier coding tasks, `qwen3-coder:30b` from the Step 5 list is a strong dedicated choice, and on the GEX131, `llama4:scout` and `gpt-oss:120b` also support tools.

Select **Work** next to **Chat** in the sidebar, choose the model, and describe a task - for example, "Create a small landing page for a bakery and start a preview". The Files, Activity, Git, Terminal, and Preview tabs let you inspect everything the model does inside the sandbox. Task files persist in a dedicated Docker volume until you delete the task.

If you do not want Work at all, remove the `/var/run/docker.sock` line from `docker-compose.yml` and run `docker compose up -d` again - the Work page will report the runtime as unavailable, and everything else keeps working.

## Step 7 - Set Up HTTPS with Caddy (Optional)
...
Now update `CORS_ORIGIN` and `BASE_URL` in your `docker-compose.yml` to match the HTTPS domain. This is required because Libre WebUI validates the origin of incoming requests - with a stale value, the page still loads but chat streaming fails:
...
## Step 8 - Verify GPU Utilization (Optional)
...
* Ran sandboxed coding tasks with Work
...
* Try larger models - upgrade to the GEX131 (96 GB VRAM) to run `llama4:scout` and beyond
* Connect cloud providers (OpenAI, Anthropic, Mistral AI) alongside Ollama for a unified interface
* For GDPR-compliant managed AI infrastructure, Kroonen AI delivers turnkey deployments for organizations worldwide - including custom model fine-tuning, SLA-backed support, and air-gapped installations
